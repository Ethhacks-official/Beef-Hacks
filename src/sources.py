...
class Sources:
    ...
    def port_forward_using_localhostrun(self,key_name,port,queue):
        url_no = 0
        try:
            if port=="":
                ports = []
            ...
            while True:
                ...
                if isinstance(json_output, dict):
                    if "address" in json_output:
                        if url_no == 0:
                            queue.put(json_output["address"])
                            url_no+=1
                        elif url_no > 0:
                            print(f"\n{RED}Localhost.run have changed the address due to some issue with localhost.run... Current urls will not work.{RESET}")
                            print(f"{RED}Press CTRL+C to quit......{RESET}")
